...
def check_forecast_availability(selected_date: str) -> dict:
    ...
    if days_ahead < 0:
        ...
        return {"available": True, "reliability": "medium",
                "message": "[!] Forecast is approximate - check again closer to your hike date"}
    elif days_ahead <= 16:
        return {"available": True, "reliability": "low",
                "message": "[!] Weather this far ahead is very uncertain - rough estimate only"}
    else:
        return {"available": False, "reliability": "none",
                "message": "[!] Weather forecast not available beyond 16 days"}
...
def compute_latest_start(duration_minutes: float, sunset_str: str) -> str:
    ...
    duration_delta = timedelta(minutes=float(duration_minutes))
...
def generate_weather_warnings(start_w: dict, summit_w: dict,
    ...
    if summit_w["rain_probability"] > 50:
        ...
        warnings.append(
            f"[!] Rain likely at summit ({summit_w['rain_probability']}%) - carry waterproof jacket")
    ...
    if summit_w["wind_kmh"] > 70:
        ...
        warnings.append(
            f"[!] Windy at summit ({summit_w['wind_kmh']:.0f} km/h) - windproof layer recommended")
    ...
    if summit_w["temp_min_c"] < 0:
        ...
        warnings.append(
            f"[!] Near freezing at summit ({summit_w['temp_min_c']}C) - insulating layer essential")
    ...
    if start_w["rain_probability"] > 35:
        warnings.append(f"[!] Rain at trailhead ({start_w['rain_probability']}%)")
    if start_w["temp_min_c"] < 2:
        warnings.append(f"[!] Cold at trailhead ({start_w['temp_min_c']}C) - dress in layers")

    if season == "winter" and max_elevation_m > 1800:
        warnings.append("[!] High altitude winter trail - crampons or microspikes needed")
    if season == "summer" and max_elevation_m > 1800:
        warnings.append("[!] Afternoon thunderstorms common above 1800m - start before 8AM")
    if season == "spring":
        warnings.append("[!] Snowmelt may make river crossings dangerous")
    if season == "autumn" and max_elevation_m > 2000:
        warnings.append("[!] First snow possible above 2000m - check conditions before going")
...
def get_full_weather_context(route_path, date: str,
    ...
    # Safety: ensure these are always floats
    duration_minutes = float(duration_minutes) if duration_minutes else 240.0
    max_elevation_m  = float(max_elevation_m)  if max_elevation_m  else 0.0
